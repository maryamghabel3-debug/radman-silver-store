...
### مرحله ۲ — اجرای نصب‌کننده (dry-run + install)
1. وارد SSH شوید.
...
3. اگر خروجی بدون خطا بود، نصب واقعی را اجرا کنید:
   ```bash
   export PATH="$HOME/bin:$PATH"
   bash /home/radmansi/radman-deploy/repo/scripts/install_agents.sh --install
   ```
   این دستور:
...
- **Command:**
  ```bash
  APP_ENV=staging WP_PATH=/home/radmansi/staging.radmansilver.ir WP_URL=https://staging.radmansilver.ir RADMAN_REPO_ROOT=/home/radmansi/radman-deploy/repo RADMAN_PRIVATE_DIR=/home/radmansi/.config/radman DRY_RUN=1 /opt/alt/python311/bin/python3.11 /home/radmansi/radman-deploy/repo/agents/agent_stock_guard.py >> /home/radmansi/.config/radman/logs/stock_guard.cron.log 2>&1
  ```
...
- **Command:**
  ```bash
  APP_ENV=staging WP_PATH=/home/radmansi/staging.radmansilver.ir WP_URL=https://staging.radmansilver.ir RADMAN_REPO_ROOT=/home/radmansi/radman-deploy/repo RADMAN_PRIVATE_DIR=/home/radmansi/.config/radman DRY_RUN=1 /opt/alt/python311/bin/python3.11 /home/radmansi/radman-deploy/repo/agents/agent_price_engine.py >> /home/radmansi/.config/radman/logs/price_engine.cron.log 2>&1
  ```
...
> محصولاتی که در حالت `legacy_mirror` یا `manual_locked` باشند، در این محاسبه **هیچ‌گاه** تغییر قیمت نمی‌خورند.
...
4. پس از اجرای بعدی Cron، اعلان‌ها به صورت SMS واقعی ارسال می‌شوند و یک کپی از هر پیامک در `outbox/sms-sent-<timestamp>.txt` ذخیره می‌شود.
   - در صورت خطا (مثل قطع بودن Kavenegar یا نبود اینترنت)، اعلان به صورت `sms-failed-<timestamp>.txt` در outbox ذخیره می‌شود تا از دست نرود.
...
| لاگ Cron | `~/.config/radman/logs/<agent>.cron.log` |
| اعلان‌های اعلان سفارش (در حالت dry) | `~/.config/radman/outbox/sms-<timestamp>.txt` |
...
4. **محدود به STAGING:** هر سه ایجنت قبل از اجرا بررسی می‌کنند که `APP_ENV=staging` و `WP_PATH` شامل `public_html` نباشد.
