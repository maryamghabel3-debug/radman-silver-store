...
- [ ] Pending : تأیید و قفل نهایی هویت بصری، لوگوی مینیمال هدر (`Primary T0`) و لوگوی بسته‌بندی (`Tagline T2`).
- [ ] Pending : تأیید قوانین ۴ گانه قیمت‌گذاری و ورود نرخ روز نقره از تلگرام (`/price 85000`).
- [ ] Pending : تأیید اصل انبار دقیق ۱ به ۱ (`stock=1` قابل فروش است، بدون بافر ریاضی).

### B. Technical & Infrastructure Readiness (`آمادگی فنی و زیرساخت`)
- [ ] Pending : پایداری سرور میزبان داخل ایران (`Iran Server Sonic 30` - `Status: TBD / PENDING`).
- [ ] Pending : صحت نصب وردپرس ۶.x، ووکامرس و قالب Blocksy Child Theme.
- [ ] Pending : صحت عملکرد وب‌سرویس ووکامرس (`WooCommerce REST API v3`) و وب‌هوک‌های خروجی.
...
- [ ] Pending : صحت کارکرد درگاه پرداخت زرین‌پال در محیط زنده و بازگشت موفق به صفحه رسید ووکامرس.
...
- [ ] Pending : صحت عملکرد ربات تلگرام مدیریت (`@RadmanSilverStoreBot`) برای دریافت دستور `/price` و نمایش پیش‌نمایش قیمت.
