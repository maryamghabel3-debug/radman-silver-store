...
### E. Messaging & Telegram HITL Readiness (`آمادگی اطلاع‌رسانی و تلگرام`)
- [ ] Pending : صحت عملکرد ربات تلگرام مدیریت (`[RADMAN_TELEGRAM_BOT_USERNAME: TBD]`) برای دریافت دستور `/price` و نمایش پیش‌نمایش قیمت (`Status: NOT YET TESTED`).
- [ ] Pending : صحت ارسال اعلان سفارش جدید به تلگرام و کارکرد دکمه‌های `[تأیید موجودی و ارسال]` و `[عدم موجودی و لغو]`.
- [ ] Pending : صحت ارسال پیامک‌های کاوه‌نگار به مشتری (کد OTP، تأیید سفارش و کد رهگیری مرسوله).
...
2. **اختلال یا از کار افتادن گردش کار تأیید انسانی در تلگرام (`Telegram Approval Flow Broken`).**
