...
- **Original-Product Pipeline Status:** Excel remains authoritative for selection, price, stock and active state. PR-33 retains PR-32 identity/scoping validation, removes the internal extraction disclaimer from public descriptions, and strips redundant `انگشتر:`/`دستبند:`/`گردنبند:` prefixes from specification values.
...
- [docs/HTML-SPEC-ENRICHMENT-RUNBOOK.md](docs/HTML-SPEC-ENRICHMENT-RUNBOOK.md) / [scripts/run_excel_import.sh](scripts/run_excel_import.sh) — PR-33 clean public descriptions, normalized spec values, PR-32 validation safeguards, and guarded staging execution for the existing 20 Drafts.
