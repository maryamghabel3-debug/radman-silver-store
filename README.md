...
- **Static Content Status:** 11 pages (IDs `21–31`) currently exist as **Draft placeholders** created during manual bootstrap. Repo content for all 11 pages is **complete and placeholder-free** in `content/static-pages/` (verified by `scripts/check_no_placeholders.py`; normal Persian ellipsis `…` is explicitly allowed). The plan runner (`scripts/radman_stage_apply.sh`, default `--plan`, staging only, never publish automatically) runs cleanly end-to-end and renders all 11 HTML fragments. Host-side Draft deployment is **PENDING** reviewer approval of a host-apply mission. **Publication is BLOCKED** pending owner operational approval (all 11 pages) and legal review (returns, privacy, terms). The approval gate is tracked in [docs/STATIC-CONTENT-APPROVAL-REGISTRY.md](docs/STATIC-CONTENT-APPROVAL-REGISTRY.md). See also [docs/STATIC-PAGES-REGISTRY.md](docs/STATIC-PAGES-REGISTRY.md) and [docs/RADMAN-BRANDING-CONTENT-IMPORT-RUNBOOK.md](docs/RADMAN-BRANDING-CONTENT-IMPORT-RUNBOOK.md).
