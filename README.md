...
- **Definitive Excel Product Source (PR-28):** `/home/radmansi/radman-deploy/products_20260821_182238.xlsx` is the single source of truth for product data. Newest selection is `legacy_id DESC` (higher ID is newer), with up to 1000 eligible rows. Catalog scraping for data is deprecated; public legacy requests remain image-gallery-only.
- **Original-Product Pipeline Status:** the prior PR-25 ten-product data-scrape path is retained only for historical/offline compatibility and is deprecated for catalog data. PR-28 uses Excel data, create-only Draft imports, real Excel stock, original-image QA/fallback, idempotent legacy IDs, and skip/report SKU conflicts. PR-29 additionally parses real per-product specification blocks in the same image-page request, reconciles missing/mismatched weights, and generates unique visible descriptions; `--enrich-existing` upgrades existing Drafts idempotently.
...
- [docs/EXCEL-1000-PRODUCT-IMPORT-RUNBOOK.md](docs/EXCEL-1000-PRODUCT-IMPORT-RUNBOOK.md) / [scripts/run_excel_import.sh](scripts/run_excel_import.sh) — Definitive Excel-driven newest-1000 selection, ID-based gallery/spec discovery, gemstone-aware Toman pricing, unique descriptions, `--enrich-existing`, private reports, and guarded Draft operations.
